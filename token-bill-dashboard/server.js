 ...
 *   opencode-go   — /workspace wrk_01KREBZW... via OPENCODE_API_KEY → opencode.ai/zen/go/v1/chat/completions
...
const OC_AUTH_KEY_PATH = "/root/.opencode/auth.key";     // optional; falls back to env
const OR_AUTH_KEY_PATH = "/root/.openrouter/auth.key";   // optional; falls back to env
...
// tiny probe: ask for 1 token so the upstream is happy to answer
const OC_GO_PROBE = {
...
async function pollOpencodeGo() {
  ...
  // 429 with GoUsageLimitError = explicit monthly cap reached
  if (resp.status === 429 && body && body.error && body.error.type === "GoUsageLimitError") {
    ...
    return setOk("opencode_go", {
      ...
      monthly: {
        ...
        note: "Monthly usage limit reached. Resets in " + humanizeSeconds(resetIn) + ". To continue using this model now, enable usage from your available balance.",
      },
...
function parseOpenRouter(key, credits) {
  const label = key.data && key.data.label ? key.data.label : null;
  ...
  return {
    ...
    limit: d.limit,                  // null for pay-as-you-go
    limitReset: d.limit_reset || null,
    limitRemaining: d.limit_remaining, // null for pay-as-you-go

...
REPORTS_LOG = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'reports_log.json')
MAX_LOG_ENTRIES = int(os.getenv('REPORT_MAX_LOG', '200'))


def _load_log() -> list:
    """Load the reports log from disk."""
...
def _save_log(entries: list):
    """Save the reports log to disk, trimming to MAX_LOG_ENTRIES."""
    trimmed = entries[-MAX_LOG_ENTRIES:]
    with open(REPORTS_LOG, 'w', encoding='utf-8') as f:
        json.dump(trimmed, f, ensure_ascii=False, indent=2)


def log_report(ticker: str, company_name: str, report: str, data: dict, elapsed_s: float):
    """Append a report entry to the persistent log."""
    entries = _load_log()
    entry = {
        'ticker': ticker,
        'company_name': company_name,
        'currency': data.get('fundamentals', {}).get('currency', 'USD'),
        'current_price': data.get('technicals', {}).get('current_price'),
        'change_pct': data.get('technicals', {}).get('change_pct'),
        'pe_ratio': data.get('fundamentals', {}).get('pe_ratio'),
        'sector': data.get('fundamentals', {}).get('sector'),
        'exchange': data.get('fundamentals', {}).get('exchange'),
...
def _extract_recommendation(report: str) -> str:
    ...
    if re.search(r'(強烈建議買入|建議買入|強力買入)', report):
        return '買入'
    if re.search(r'(建議賣出|賣出|減持)', report):
        return '賣出'
    return '持有'


def _extract_score(report: str) -> str:
    """Extract score like 7/10 from report."""
    m = re.search(r'(\d+(?:\.\d+)?)\s*/\s*10', report)
    if m:
...
def generate_report(data: dict) -> str:
    ...
    system_prompt = (
        ...
        "- 非常重要：必須完成所有七個章節，特別係最後嘅「## 綜合評分」。唔可以因為字數而 skip。\n"
    )
    ...
    # Increase timeout: LLM generation can take 3-5 min with long reports
    ...
    def do_POST(self):
        ...
        if self.path == '/stock-report/api/analyze':
            try:
                ...
                elapsed_fetch = round(time.time() - t_fetch_start, 1)
                ...
                if cached_report:
                    ...
                    elapsed_llm = round(time.time() - t_llm_start, 1)

                total_elapsed = round(time.time() - t_start, 1)

                # Persist to log
                log_report(ticker, data['fundamentals']['name'], report, data, total_elapsed)
        ...
        if self.path == '/stock-report/api/history':
            try:
                entries = _load_log()
                # Return summary (no full report text in list)
                summaries = []
                ...
                return send_json(self, {'ok': True, 'count': len(summaries), 'entries': summaries})
            except Exception as e:
                return send_json(self, {'ok': False, 'error': str(e)}, 500)

        if self.path == '/stock-report/api/report':
            try:
                length = int(self.headers.get('Content-Length', '0'))
                body = json.loads(self.rfile.read(length).decode('utf-8')) if length else {}
                ticker = str(body.get('ticker', '')).strip()
                if not ticker:
                    return send_json(self, {'ok': False, 'error': 'Missing ticker'}, 400)
                entries = _load_log()
                for e in entries:
                    if e['ticker'].upper() == ticker.upper():
                        return send_json(self, {'ok': True, 'entry': e})
                return send_json(self, {'ok': False, 'error': f'No saved report for {ticker}'}, 404)
            except Exception as e:
                return send_json(self, {'ok': False, 'error': str(e)}, 500)

...
The login scene uses a hybrid environment: an original 2560×1440 painted Chinese fantasy city provides the distant world, while a transparent Godot `SubViewport` renders detailed foreground buildings, tiled roofs, timber brackets, balconies, lattice windows, stone paving, lanterns, shader materials, fog, banners, and moving cloud layers in real time. No pedestrians are used. `scripts/wuxia_ost.gd` synthesizes the original pentatonic soundtrack at runtime, so the project does not ship third-party music.
